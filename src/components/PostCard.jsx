...
const PostCard = ({ post, onDeleted }) => {
  ...
  useEffect(() => {
    if (isOwnPost) {
      setResolvedCat({
        avatar: currentUser.activeCat?.avatar || post.cat?.avatar || '/favicon.svg',
        name: currentUser.activeCat?.name || post.cat?.name || '—',
      });
      return;
    }
    ...
    getCachedUsers().then(users => {
      ...
      if (!author) return;
      setResolvedCat({
        avatar: author.activeCat?.avatar || post.cat?.avatar || '/favicon.svg',
        name: author.activeCat?.name || post.cat?.name || '—',
      });
    }).catch(() => {});
  }, [post.userId, isOwnPost, currentUser?.activeCat?.avatar, currentUser?.activeCat?.name]);
      ...
      <div className="bg-white rounded-lg shadow-sm mb-3 border border-[#dddfe2] overflow-hidden relative">
        ...
        <div className="px-4 pt-3 pb-2 flex items-center justify-between">
          <div className="flex items-center gap-2 min-w-0 cursor-pointer" onClick={() => post.cat && goToProfile(post.cat)}>
            <img src={resolvedCat.avatar} alt="Author" className="w-10 h-10 rounded-full object-cover shrink-0 hover:opacity-90 transition-opacity" />
